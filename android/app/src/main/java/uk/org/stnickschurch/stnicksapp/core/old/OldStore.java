...
public class OldStore {
    ...
    private OldStore(Context context) {
        mContext = context;
    ...
    public void sync() {
        Downloader.SINGLETON.get(mContext)
                .getRequest(mContext.getString(R.string.old_sermon_list), null)
                .observeOn(Schedulers.io())
    ...
    public static final Singleton<OldStore> SINGLETON = new Singleton<OldStore>() {
        @Override
        protected OldStore newInstance(Context context) {
            return new OldStore(context);
        }
    };
}
